...
def demonstrate_similarity_search():
    ...
    for movie_id in test_movies:
        ...
        if "error" not in reference:
            ...
            print(f"   🎭 Genres: {', '.join(reference['genres'])}")
    ...
    for scenario in scenarios:
        ...
        if results.get('results'):
            for movie in results['results']:
                ...
                print(f"   • {movie['title']} ({movie['release_year']}) - ⭐{movie['rating']}{score_info}")
                print(f"     🎭 {', '.join(movie['genres'])}")
        else:
...
def main():
    ...
    try:
        ...
        if not client.index_exists():
            print("❌ Movie index not found. Please run 'python main.py' first to set up the database.")
            return
        
        print("✅ Connected to Elasticsearch and found movie index")
